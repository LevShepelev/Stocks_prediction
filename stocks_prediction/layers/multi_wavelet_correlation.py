...
import pdb
from stocks_prediction.utils.masking import LocalMask
...
class MultiWaveletCross(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, seq_len_q, seq_len_kv, modes, c=64,
        ...
        self.attn2 = MultiWaveletTransform(ich=ich_dim, k=self.k, c=self.c, L=self.L, base=base)
        self.attn3 = MultiWaveletTransform(ich=ich_dim, k=self.k, c=self.c, L=self.L, base=base)
        self.attn4 = MultiWaveletTransform(ich=ich_dim, k=self.k, c=self.c, L=self.L, base=base)
...
class sparseKernelFT1d(nn.Module):
    ...
    def __init__(self, k: int, alpha: int, c: int = 1,
                 nl: int = 1, initializer=None, **kwargs):
        super().__init__()
        self.k = k
        self.modes1 = alpha
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        out_ft[..., :l] = self.compl_mul1d(
            x_fft[..., :l],          # (B, in_ch, l)
            self.weights1[..., :l]   # (in_ch, out_ch, l)
        )
